 ...
 */
static int32_t adc_read_raw(print_string_t __print_string, const char *message, uint8_t force_wait)
{
	...
	if (force_wait) {
		...
		while (ADC_DATA_PIN & (1 << ADC_DATA_NUM)) {
            // Optionally print message during wait
            if (__print_string && message) {
                __print_string(message);
            }
        }
	} else {
 ...
 */
static int32_t adc_read_averaged(int32_t times, print_string_t __print_string, const char *message)
{
    int32_t total = 0;
    uint8_t idx = 0;
    ...
    if (times <= 3) {
        for (idx = 0; idx < times; idx++) {
            total += adc_read_raw(__print_string, message, 1);
        }
    ...
    for (idx=0; idx<times && idx<AVG_SAMPLE_COUNT; idx++) {
        readings[idx] = adc_read_raw(__print_string, message, 1);
    }
 ...
 */
static int32_t adc_read_filtered()
{
    // Read new raw ADC data
    int32_t raw_data = adc_read_raw(NULL, NULL, 0);
    ...
    if (delta > SP_CHANGE_DETECT_THRESHOLD) {
        ...
        int32_t confirm_data = adc_read_raw(NULL, NULL, 0);
 ...
 */
static inline int32_t adc_read_with_offset()
{
    return adc_read_filtered() - sp_zero_offset;
}

int32_t adc_read_calibration(print_string_t __print_string, const char *message)
{
    return adc_read_averaged(AVG_SAMPLE_COUNT, __print_string, message) - sp_zero_offset;
}

/**
 * Perform tare operation (set current reading as zero)
 */
void scale_tare_zero(print_string_t __print_string, const char *message, uint8_t first_call)
{
    if (first_call) {
        sp_zero_offset = adc_read_averaged(AVG_SAMPLE_COUNT, __print_string, message);
    } else {
        sp_zero_offset = adc_read_filtered();
    }
 ...
 */
static inline void scale_set_gain(uint16_t gain)
{
    ...
    ADC_CLK_PORT &= ~(1<<ADC_CLK_NUM);
    adc_read_raw(NULL, NULL, 1);
}

/**
 * Initialize hardware pins and default settings
 */
